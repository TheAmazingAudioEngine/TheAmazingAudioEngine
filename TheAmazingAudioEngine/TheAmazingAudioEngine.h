...
@page Audiobus Audiobus
 ...
 - Receive Audiobus audio by creating an ABReceiverPort, adding it to the ABAudiobusController, then
   passing it to The Amazing Audio Engine via AEAudioController's 
   [audiobusReceiverPort](@ref AEAudioController::audiobusReceiverPort) property.
 - Send your app's audio output via Audiobus by creating an ABSenderPort, adding it to the ABAudiobusController,
   then assigning it to [audiobusSenderPort](@ref AEAudioController::audiobusSenderPort).
 ...
 - Filter Audiobus audio by creating an ABFilterPort with AEAudioController's [audioUnit](@ref AEAudioController::audioUnit),
   adding it to the ABAudiobusController, then passing it to The Amazing Audio Engine via AEAudioController's 
   [audiobusFilterPort](@ref AEAudioController::audiobusFilterPort) property.
